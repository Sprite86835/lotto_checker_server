...
def check_lotto():
    if 'image' not in request.files or 'numbers' not in request.form:
        return jsonify({'error': 'Bild oder Zahlen fehlen'}), 400
    ...
    for i in range(len(data['text'])):
        ...
        if text.isdigit():
            ...
            color = (0, 0, 255) if text in user_numbers else (180, 180, 180)
            cv2.rectangle(img_cv, (x, y), (x + w, y + h), color, 2)
            cv2.putText(img_cv, text, (x, y - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 1)

    # Bild als JPEG zurücksenden
